...
async bookListDeliveryPigeon40(enteredSearch, searchBatch) {
    const booksPigeonDelivered = await fetch(baseURL + `?q=search ${enteredSearch}
    &printType=books&maxResults=40&startIndex=${searchBatch}&key=` + key)
    const booksPigeonTranslated = await jsonTranslationPigeon(booksPigeonDelivered)
